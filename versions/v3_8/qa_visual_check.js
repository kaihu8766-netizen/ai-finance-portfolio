    ...
    test(`[${vp.name}]`, async ({ page }) => {
      ...
      if (!navFailed) {
        ...
        const stack = await page.evaluate(() => {
          ...
          if (!sec) return null;
          const groupEls = [...sec.querySelectorAll('div[style*="flex:1 1 30%"]')];
        ...
        c.check('技术栈 3 组卡片', stack && stack.groups === 3 && ['通信层', '质检层', '风控层'].every(n => stack.names.some(x => x.includes(n))),
...
test.describe('内容断言 v3.6/v3.7/v3.8', () => {
  test('v3.6/v3.7/v3.8 全量内容 + 能力全景响应式', async ({ page }) => {
    ...
    const prodCap = await page.evaluate(() => {
      ...
      return {
        hasDesign: text.includes('产品设计：SmartRecon 对账 Agent · SOX 控制测试工作台 · 月结流程优化方案 · 收入核算产品方案'),
        hasTech: text.includes('技术实现：Python / SQL / Power BI 数据分析 · AI Agent / RAG / MCP 系统设计'),
      };
    });
    ...
    const stackGroups = await page.evaluate(() => {
      ...
      if (!sec) return null;
      const text = sec.textContent;
      return { hasComm: text.includes('通信层'), hasQA: text.includes('质检层'), hasRisk: text.includes('风控层'), hasBridge: text.includes('Bridge Protocol'), hasPlaywright: text.includes('Playwright'), hasFuse: text.includes('Bug 熔断') };
    });
    c.check('N6 技术栈 3 组卡片齐全', stackGroups && stackGroups.hasComm && stackGroups.hasQA && stackGroups.hasRisk && stackGroups.hasBridge && stackGroups.hasPlaywright && stackGroups.hasFuse,
      stackGroups ? `comm=${stackGroups.hasComm} qa=${stackGroups.hasQA} risk=${stackGroups.hasRisk}` : '缺失');
    ...
    const stackClean = await page.evaluate(() => {
      ...
      if (!sec) return null;
      const text = sec.textContent;
      return { hasGitHub: text.includes('GitHub Skills'), has57: text.includes('57 封') || text.includes('57封') };
    });
