...
test.describe('内容断言 v3.6/v3.7/v3.8', () => {
  test('v3.6/v3.7/v3.8 全量内容 + 能力全景响应式', async ({ page }) => {
    ...
    // N10：v3.10 商业化深度（G2N / CCC / DLQ+AuditLog）
    const commercial = await page.evaluate(() => {
      ...
      return {
        w03G2N: w03.includes('Gross-to-Net') && w03.includes('净收入还原') && w03.includes('GMV'),
        w03NoFake: w03.includes('产品方案设计，非生产数据'),
    ...
    c.check('N10a w03 G2N 净收入还原链路', commercial && commercial.w03G2N,
      commercial ? `G2N=${commercial.w03G2N} noFake=${commercial.w03NoFake}` : '缺失');
    ...
    const kbJson = await page.evaluate(() => {
      ...
      try {
        ...
        const keyOk = kb.keyNumbers && kb.techStack && kb.defectLog && kb.versions.includes('v3.11');
    ...
    c.check('N11 v3.11 KB 独立 JSON 块 + 引擎加载', kbJson && kbJson.ok,
    ...
    dump('内容断言 v3.6/v3.7/v3.8/v3.10/v3.11', c);
  });
});
