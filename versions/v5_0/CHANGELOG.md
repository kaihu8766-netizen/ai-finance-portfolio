...
| 8 | **Hero 区深度更新** | 副标题"双 Agent 系统落地"→"四角色 AI 协作系统落地"；统计数字 2/12/98.1% → 4 四角色协作 / 10K 蒙特卡洛路径 / 3 可运行 Demo（精算能力前置） |
| 9 | **能力四宫格更新** | AI Agent 系统描述：旧版"自定义通信协议+质检流水线+熔断自检"→"四角色协作体系——决策·架构·构建·审计，Bridge 信箱通信+强制讨论+Bug熔断+视觉门禁" |
| 10 | **关于我更新** | AI 工具链 WorkBuddy 角色：Builder & Archive → Architect（四角色体系架构师），描述同步更新 |
| 11 | **AI引擎注释** | "智对账 AI 引擎" → "作品集 AI 引擎" |
| 12 | **面试官模式** | 考察范围：旧关键词(对账/月结/收入核算) → 当前作品关键词(现金流压力测试/精算建模/LPR预测/供应链金融/SOX)；AI能力加入四角色协作/MCP |
| 13 | **AI助手离线导览** | 作品编号重新梳理；"3层主线" → "四层递进主线" |
| 14 | **AI助手迭代历史** | "双Agent协作实战展示" → "协作实战展示 → v5.0四角色协作体系升级" |
| 15 | **portfolioKB主页** | "3层主线故事(研究→产品→系统)" → "四层递进主线(研究→AI系统→审计→精算风险建模)"；highlights同步更新 |
| 16 | **快捷提问** | "财务数据看板怎么做？" → "精算建模怎么做的？" |
| 17 | **作品排序** | wide卡顺序 w05→w06→w07 改为 w07(精算核心)→w05→w06，精算作品前置 |
| 18 | **Hero标题** | "到 AI Agent 落地" → "到 精算建模 × AI Agent 落地"，精算元素前置到标题 |
| 19 | **w01标签** | RESEARCH → RESEARCH · 基础层，明确层级定位 |
| 20 | **能力四宫格** | 资金运营："SAP实操·融资全流程·资金看板" → "SAP资金模块·银团授信/外汇结算·资金头寸看板"，与smart实习具体工作对齐 |
| 21 | **滚动进度条** | 页面顶部新增2px渐变进度条，随滚动实时填充 |
| 22 | **回到顶部按钮** | 右下角新增悬浮按钮，滚动超600px淡入，点击平滑回顶 |
| 23 | **统计数字计数动画** | Hero区5/4/10K/3滚动到视口时从0动态增长，IntersectionObserver触发 |
| 24 | **作品卡片hover增强** | wide卡hover发光增强，边框主色高亮 |
| 25 | **详情页背景遮罩** | 打开作品详情时加半透明黑色遮罩+背景模糊，点击遮罩/ESC可关闭 |
| 26 | **AI助手脉冲动画** | 悬浮按钮加呼吸光晕，2.5s循环，暗示可交互 |
| 27 | **四角色区域hover** | about-card hover上浮+边框高亮 |
| 28 | **能力四宫格图标动画** | hover时图标缩放1.15+旋转-5度，卡片上浮 |
| 29 | **Hero动态背景** | 新增radial-gradient流动动画，12s循环，深色背景不单调 |
| 30 | **导航栏增强** | "方法论"→"四角色协作"；滚动时加阴影增强 |
| 31 | **w07精算核心卡** | 加actuarial-core class，绿色渐变背景+hover绿色发光 |
| 32 | **ESC关闭详情页** | 按ESC键可关闭作品详情页 |
| 33 | **w07 可交互模拟器** | 新增 `demo/现金流压力测试模拟器.html`：9个参数滑块实时调节（期初余额/路径数/回款率/销售增速/波动率/采购支出/融资利率/经营流入/安全垫），纯JavaScript蒙特卡洛引擎（Box-Muller正态+Marsaglia-Tsang Gamma近似Beta+Vasicek+GBM+Cholesky相关性），10000次模拟0.3秒，输出4项KPI+3张ECharts图表+6条AI调度建议 |
| 34 | **w07 踩坑与技术实现模块** | 详情页新增sec06：4个真实踩坑（独立抽样相关性丢失→Cholesky分解/极端情景余额为负→融资约束/Beta抽样性能差→Marsaglia-Tsang算法提速6倍/Beta参数无解析解→矩估计R²0.71→0.89）+技术实现栈表格+量化成果；原sec06局限顺延为sec07 |
| 35 | **w07 Demo入口** | 数据看板区新增绿色高亮Demo入口条，一键打开可交互模拟器 |
...
- w07 可交互模拟器：9滑块/4KPI/3图表/6条AI建议全部正常，10000次模拟运行成功 ✅
- w07 详情页：Demo入口/踩坑模块/技术栈/sec07局限全部渲染正常，3张ECharts图表正常 ✅
