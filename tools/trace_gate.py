...
# 作品集仓内 trace/ 决策追踪目录
TRACE = ROOT / "trace"
# 评审档案相对作品集仓根的目录（git log -- <path> 用）
ARCHIVE_DIR = "trace/03-会议与日志/DeepSeek评审"
RV_DIR = ROOT / ARCHIVE_DIR
...
ID_RE = re.compile(r"\b(?:GATE|PF-RV|RV|DEC|ISS)[-:]\d{6,8}(?:-\d+)?\b|\bISS-\d+\b")
...
def _find_ids(text: str, prefix: str) -> list[str]:
    pat = rf"\b(?:PF-)?{prefix}-\d{{6,8}}(-\d+)?\b|\b(?:PF-)?{prefix}-\d+\b"
...
def cmd_check(message: str) -> int:
    ...
    if not ids:
        print("[check] FAIL：commit message 必须含 ID 引用（GATE-* / PF-RV-* / DEC-* / ISS-*）")
        print("[check] 示例：feat(scope): 描述 (PF-RV-20260923-01, GATE-20260923-01)")
        return 1
    # 域前缀强制（F-20260924-02，裸 RV- 仅协作A专用，作品集必须用 PF-RV-）
    for rid in ids:
        if rid.startswith("RV-"):
            print(f"[check] FAIL：裸 {rid} 仅协作A专用，作品集必须使用 PF-RV- 前缀（域前缀即全局唯一）", file=sys.stderr)
            return 1
    # 存在性校验：PF-RV 号必须在 trace/ 索引中真实存在（防伪造号自证）
    idx = _read_text(RV_INDEX)
    for rid in ids:
        if rid.startswith("PF-RV-"):
            # PF-RV-YYYYMMDD-NN → 档案名前缀 YYYY-MM-DD-NN-
            ...
            if m:
                ...
                if prefix not in idx:
                    ...
                    return 1
    print(f"[check] OK：引用 {ids}（域前缀+存在性校验通过）")
    return 0
...
def _classify(staged_diff: str) -> tuple[list[str], str]:
    ...
    HARD_GATE_SELF = ("tools/trace_gate.py", ".githooks/commit-msg", "tools/gate_rules.yaml",
                      "tools/deepseek_gate.py", "PROJECT_BRIEF.md", "REVIEW_STAMP.md")
...
def cmd_check_rv(message: str) -> int:
    ...
    if not hits:
        ...
        return 0
    m = re.search(r"\b(?:PF-RV|RV)-(\d{6,8}(?:-\d+)?)\b", message)
    if not m:
